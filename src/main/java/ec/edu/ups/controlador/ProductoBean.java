...
public class ProductoBean implements Serializable {
    ...
    public void addProducto() {
        ...
        Producto s1 = new Producto(nombre.toUpperCase(), imagen, Double.parseDouble(precioCompra), Double.parseDouble(precioVenta), iva_char, Integer.parseInt(stock), categoria_buscada);
...
//FacesContext.getCurrentInstance().getApplication().getNavigationHandler().handleNavigation(FacesContext.getCurrentInstance(), null, "paginaBodegas.xhtml")
    ...
    public void redirectBodegas(){
        try {
            FacesContext.getCurrentInstance().getExternalContext().redirect("/Proyecto-1.0-SNAPSHOT/private/paginaBodegas.xhtml");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
